...
-- 연습 1) 10번 부서(번호)의 소속 사원을 출력하세요 (전체 출력 : *)
-- 사원 테이블 : EMPLOYEE
-- 부서번호 컬럼 : DNO
-- 출력 컬럼 : *
-- 조건절 : WHERE DNO = 10;
...
-- 연습 2) 사원테이블에서 월급이 5000인 사람의 이름을 출력하세요
SELECT ENAME FROM EMPLOYEE
WHERE SALARY = 5000;

-- 예제 2) 사원명이 SCOTT인 사원을 출력하세요 (전체출력)
SELECT * FROM EMPLOYEE
WHERE ENAME = 'SCOTT';

-- 연습 3) 사원명이 ALLEN인 사원을 출력하세요 (전체출력)
SELECT * FROM EMPLOYEE
WHERE ENAME = 'ALLEN';

-- 연습 4) JOB(직위)이 CLERK인 사원을 출력하세요 (전체출력)
SELECT * FROM EMPLOYEE
WHERE JOB = 'CLERK';

-- 예제 3) 입사일(HIREDATE)이 '1981/01/01' 이전인(이하) 사원만 출력하기 (전체 출력)
-- HIREDATE  날짜 자료형 비교도 문자열과 같이 비교하면 됨
SELECT * FROM EMPLOYEE
WHERE HIREDATE <= '1981/01/01';

-- 연습 5) 입사일이 '1987/01/01' 이상인 사원만 출력하기 (전체 출력)
SELECT * FROM EMPLOYEE
WHERE HIREDATE >= '1987/01/01';

-- *) 논리연산자와 함꼐 조건절(Where) 사용하기
-- 복습 : 논리연산자 : AND(논리곱, 이고, 그리고), OR(논리합, 이거나, 또는), NOT(부정, 반대)
-- 조회 속도(성능) : AND >>>>>>> OR (실무에서도 거의 AND를 사용함)
-- 예제 4) 부서번호가 10이고(AND) 직급이 'MANAGER'인 사원을 출력하세요
SELECT * FROM EMPLOYEE
WHERE DNO = 10
AND JOB = 'MANAGER';
...
-- 연습 10) 부서번호가 10 ~ 20 번 사이에 있는 사원들을 출력하세요, 단 BETWEEN ~ AND 사용하세요
SELECT * FROM EMPLOYEE
WHERE DNO BETWEEN 10 AND 20;

-- *) NOT BETWEEN 예약어
-- 예제 7) 급여가 1000미만이거나 1500초과인 사원 조회하기
-- 1) 비교연산자 사용
SELECT * FROM EMPLOYEE
WHERE SALARY < 1000
OR SALARY > 1500;
...
-- *) IN 예약어
-- 예제 8) 상여금이 300 이거나 500 이거나 1400인 사원 조회하기
-- 1) 비교연산자 사용
SELECT * FROM EMPLOYEE
WHERE COMMISSION = 300
OR COMMISSION = 500
...
-- 연습 12) 부서번호가(DNO) 10, 20 인 사원들 조회하기(전체조회: *), 단 IN 예약어 사용하세요
-- 사원 테이블 : EMPLOYEE
SELECT * FROM EMPLOYEE
WHERE DNO IN (10,20);
...
-- => 반대 : Not (300이거나(or) 500이거나 1400이거나)
-- 사원테이블 : EMPLOYEE
SELECT * FROM EMPLOYEE
...
-- 예) 이름이 K로 시작하는 사원들 조회
-- 예제 10) 이름이 'F'로 시작하는 사원 조회하기
-- 사용법 : SELECT 컬럼명, ... FROM 테이블명
--          WHERE 컬럼명 LIKE '문자%';
...
-- 예제 11) 이름의 두번쨰 글자가 'A'인 사원 조회하기
-- LIKE 기호 : & (어떤 문자열을 의미)
--             _ (어떤 한 문자를 의미)
SELECT * FROM EMPLOYEE
WHERE ENAME LIKE '_A%';

-- 연습 15) 이름의 세번째 글짜가 'A' 인 사원 조회하기
-- 사원 테이블 : EMPLOYEE
SELECT * FROM EMPLOYEE
WHERE ENAME LIKE '__A%';

-- *) NOT LIKE 예약어(*)
-- BETWEEN, IN 앞에 NOT = 부정의 의미, 마찬가리조 LIKE 앞에 NOT 붙이면 반대(부정)의 의미
-- 예제 12) 이름에 'A' 가 포함되지 않는 사원을 조회
-- 사원 테이블 : EMPLOYEE
SELECT * FROM EMPLOYEE
...
-- 예제 13) 상여금(COMMISSION) 이 NULL인 사원을 조회
-- 사원 : EMPLOYEE
...
-- IS NULL : NULL인 정보만 조회
-- IS NOT NULL : NULL이 아닌 정보만 조회
SELECT * FROM EMPLOYEE
WHERE COMMISSION IS NULL; -- O (결과 나옴)

-- 예제 14) 상여금이 NULL이 아닌 사람 조회
SELECT * FROM EMPLOYEE
WHERE COMMISSION IS NOT NULL;

-- *) ORDER BY : 정렬 기능
-- 사용시 : 속도 저하
-- 예제 15) 사원 테이블 오름차순 정렬하기
-- 단, 월급 낮은 순으로 정렬하세요
...
-- 연습 18) 사원테이블에서(EMPLOYEE) 급여는(SALARY) 내림차순으로 정렬하고,
--         급여가 동일하면 
