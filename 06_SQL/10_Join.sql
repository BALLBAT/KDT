...
--                         2) 네트웍 통해 db 서버로 전송됨
-- 예제 1) 사원번호(ENO)가 7788인 사원의 부서명(DNAME)은 뭘까요?
-- 사원 : EMPLOYEE
-- 부서 : DEPARTMENT
...
-- 연습 1) 사원번호가 7499인 사원들에 소속된 부서정보를 모두 출력하세요
-- 부서 : DEPARTMENT
-- 사원 : EMPLOYEE
...
-- 연습 2) 사원번호가 7499 또는 7900 사원들에 소속된 부서정보를 모두 출력하세요
-- 부서 : DEPARTMENT
-- 사원 : EMPLOYEE
-- 부서컬럼 : 부서번호(DNO), 부서명(DNAME), 부서위치(LOC)
-- 힌트 : 컬럼명 IN ()
...
AND DE.DNAME LIKE 'SALES';

-- (참고) 그 밖에 특수한 조인들
-- 1) (참고) 조건없이(공통컬럼 조인없이) 조인하기 : 카티션 프로덕트(곱(하기) 조인)
-- 단점 : 건수가 늘어사너 성능 대폭 저하 (테이블1*테이블2 = 14*4 = 56)
-- 조인은 되지만 성능이 대폭 저하됨
SELECT EM.*, DE.*
FROM EMPLOYEE   EM
    ,DEPARTMENT DE;
...
-- 예제 2) 급여 등급 테이블(SALGRADE)의 낮은 등급(LOSAL) ~ 높은 등급(HISAL) 사이의 급여에 대해서
-- 사용법) SELECT 테이블1.*, 테이블2.*
--         FROM 테이블1
--            , 테이블2
...
FROM EMPLOYEE EM, SALGRADE SA
WHERE EM.SALARY BETWEEN SA.LOSAL AND SA.HISAL;

-- 3) 아우터 조인(OUTER JOIN)(*) :
-- 이퀄 조인(=) : 두 테이블의 데이터가 같은 성격을 가진 것(공통 컬럼)을 연결하는 것
-- 특징 : 두 테이블 공통컬럼에 null 값이 있으면 null은 제외하고 연결됨
--  예) 부서(Department, dno(부서번호), 사원(Employee, dno(부서번호)))
-- 아우터 조인 사용 why? 조인을 하되 null 값을 포함해서 조회하고 싶을 때 사용
-- 예제 4) 같은 테이블을 조인할 수 있음(셀프 조인)
-- 용도 : NULL을 포함한 데이터도 화면에 표시하고 싶을 때 사용
-- 사용법 : (=)이퀄 조인을 하되 NULL 값이 있는 쪽에 (+) 붙이면 아우터 조인됨
SELECT EM.ENAME,
       MA.ENAME
FROM EMPLOYEE EM,
     EMPLOYEE MA
...
-- *) 조인시 조회 성능 팁 : 1) 인덱스가 컬럼에 생성된 것끼리 이퀄조인을 하면 성능 향상
--                          2) 조인시 FROM 큰 테이블,
--                          3)             작은 테이블
--                             WHERE 큰테이블.공통컬럼 = 작은테이블.공통컬럼

-- 4) SQL-99 표준 표기법) DB SQL 명령어를 세계 기구에서 표준을 제정함
-- 4-1) 오라클 조인 : 부서 x 사원
SELECT EM.*, DE.*
FROM EMPLOYEE EM,
     DEPARTMENT DE
...
FROM EMPLOYEE EM,
     EMPLOYEE MA
WHERE EM.MANAGER = MA.MANAGER(+);
-- SQL-99 조인 : (+) 없는 쪽을 의미 => LEFT OUTER JOIN
SELECT EM.ENAME,
       MA.ENAME
