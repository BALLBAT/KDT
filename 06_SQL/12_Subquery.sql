-- 12_Subquery.sql(*)
-- 서브쿼리 : sql 안에 sql이 있는 명령문
-- 예제 1) SCOTT 사원 보다 급여를 많이 받는 사원이 누구인지 출력
...
-- 그 밖의 특수한 서브쿼리 사용 예시
-- 1) SELECT 예약어 다음에도 서브쿼리 사용 가능 : (서브쿼리)
-- 단점 : 성능 저하
SELECT 2*3, (SELECT 4*3 FROM DUAL)
FROM DUAL;

-- 2) FROM 에약어 다음에도 서브쿼리 사용 가능
SELECT EM.ENAME, EM.ENO
FROM (SELECT ENAME, DNO FROM EMPLOYEE) EM;

-- 4) 다중 행 서브쿼리 : 서브쿼리의 결과로 여러건이 나옴
-- 사용법 : SELECT 컬럼명 FROM 테이블1
--          WHERE 비교컬럼 IN (SELECT 비교컬럼
--          FROM 테이블2);
-- 주의점 : (서브쿼리)의 결과가 여러건일 때 IN 연산자를 사용해야함
