...
-- 부서테이블 : DEPARTMENT
-- 힌트) HAVING 사용
SELECT DEP.DNAME, EMP.JOB, SUM(SALARY) AS 급여총액
FROM EMPLOYEE EMP,
     DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO
...
-- 그룹핑 없음 : 그냥 조인 + 조건 쿼리
SELECT DEP.DNO, DEP.DNAME, EMP.ENO, EMP.ENAME, SALARY
FROM EMPLOYEE EMP,
     DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO
AND EMP.SALARY > 2000;
...
SELECT DEP.DNO, ROUND(AVG(SALARY)), MAX(SALARY), MIN(SALARY), COUNT(*)
FROM EMPLOYEE EMP,
     DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO
GROUP BY DEP.DNO;