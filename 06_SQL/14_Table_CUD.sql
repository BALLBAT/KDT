...
-- C : Create (생성, 추가 == Insert 문)
-- R : Read or Retieve (조회, 검색 == Select 문)
-- U : Update (수정 == Update 문)
-- D : Delete (삭제 == Delete 문)
-- 테이블 의미 : DB(Data Base, 데이터 베이스)에 자료를 저장하는 최소 단위
-- 계정 : 개발자 계정(scott 계정)
-- SQL문 : 특징 : 1) 대소문자 구분 없음

-- 예제 1) 테이블 생성 : DEPT (부서)
-- *) DB 자료형
--    NUMBER(자리수) : 실수, 정수 등을 나타내는 자료형
--    자리수 : 숫자의 자리수를 의미
--     예) NUMBER(2), NUMBER - 자리수 제한 X
--         NUMBER(7,2) -> 정수 7자리, 소수점 2자리수 숫자가 들어옴
--    VARCHAR2(자리수) : 문자열형을 나타내는 자료형 
--    자리수 : 문자열의 자리수를 의미
...
-- 실무 예 : 테이블 만들기 #2 : 테이블 복사(COPY)
-- 에제 2) DEPARTMENT 테이블 복사하기 : DEPT_COPY 테이블 생성
-- 2-1) 테이블 생성 + 데이터 복사
CREATE TABLE DEPT_COPY
AS
SELECT * FROM DEPARTMENT;

-- 2-2) 테이블 생성 (BUT 데이터 복사 안함)
...
-- 1) 테이블 주석 달기 :
-- 사용법 : COMMENT ON TABLE 테이블명 IS '테이블 주석'; 
...
-- 데이터 확정 : DB에 영구반영 (취소 안됨)
-- 확정을 해야 다른 계정(유저)이 이 데이터를 볼 수 있음(추가/수정/삭제 모두 같음)
COMMIT;

-- 예제 7) INSERT 할 때 일부 컬럼을 제외하고 추가하기
