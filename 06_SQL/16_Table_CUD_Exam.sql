...
VALUES(1, 'GMS', 'STUDENT', NULL, SYSDATE, 2700, 230, 10);
...
VALUES(2, 'PHS', 'STUDENT', NULL, TO_DATE(SYSDATE-1, 'YYYY/MM/DD'), 2500, 150, 10);
...
--   ( 대상테이블 : EMP_COPY )
-- 힌트) UPDATE 테이블명 SET 컬럼명 = 값 WHERE 조건;
UPDATE EMP_COPY
SET
DNO = 10
WHERE ENO = 7788;
...
-- 1) 사원번호 7499의 담당 업무 : SALESMAN
SELECT JOB FROM EMP_COPY
WHERE ENO = 7499;

-- 2) 사원번호 7499의 급여 : 1600
