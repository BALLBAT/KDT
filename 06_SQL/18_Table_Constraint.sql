...
-- 제약 조건 : 기본적으로 컬럼별로 각각 지정할 수 있음
-- 1) UNIQUE(유일한) 제약 조건 : 유일한 값만 입력될 수 있음(중복 금지), NULL 값은 허용
-- 1-2) NOT NULL 제약 조건(*) : 입력될 값으로 NULL 값은 허용안됨
...
--         그리고 PWD, NAME 에는 NOT NULL 제약 조건을 추가하세요
...
-- 특징 : 유일한 값만 입력됨(중복 금지) + NOT NULL 제약 조건 포함 (UNIQUE + NOT NULL)
-- (참고) : INDEX도 자동 생성(조회 속도 향상)
-- 실무) 주로 테이블 설계시 PK(기본키) 1개는 거의 대부분 생성함
-- 사용법 : CREATE TABLE 테이블명(
--          컬럼명 자료형(자리수)
--          ...,
...
-- 3) 외래키 제약 조건(*) : FOREIGN KEY(FK : 약어)
-- 참조키 : 부서테이블에 부서번호가 10 ~ 40 4건이 있음 : 부모테이블
--          사원테이블에 부서번호는 10 ~ 30 3건이 있음 : 자식테이블
-- 부모테이블 범위에 있는 것들만 들어올 수 있고 아닌게 들어오면 ERROR 발생
-- 관계 : 1) 회사 소속 사원들은 무조건 1개의 부서에 소속됨
--        2) 회사에 있는 부서에만 소속이 될 수 있음
--        => 부서(기준 : 부모역할) -> 사원(소속 : 자식역할)
-- 참조키 특징 : 1) 부모테이블의 컬럼의 데이터만 자식테이블의 컬럼 데이터로 입력될 수 있게 제약을 거는 것
--              2) 부모테이블의 해당 컬럼은 유일한 조건이어야함(기본키, UNIQUE 제약 조건)
--              3) 삭제할 때는 자식테이블 데이터 먼저 삭제하고 -> 부모테이블 데이터 삭제
-- (참고) : 참조키를 만들어서 프로젝트를 하기도 하고, 안만들고 하기도 함
-- 사용법 : CREATE TABLE 테이블명(
--            컬럼명 자료형(자리수) CONSTRAINT 제약조건명 PRIMARY KEY,
--            컬럼명2 자료형(자리수) CONSTRAINT 제약조건명 REFERENCES 부모테이블명
-- )
...
-- 기타 제약 조건 :
-- 4) CHECK 제약 조건(참고) : CK(약어)
-- 정의 : 컬럼에서 허용 가능한 데이터 범위 또는 조건을 지정할 수 있음
-- 사용법 : CREATE TABLE 테이블명(
--            컬럼명 자료형(자리수) CONTRAINT 제약조건명 CHECK(조건식) 
--             );
CREATE TABLE EMP_SECOND(
    ENO NUMBER(4) CONSTRAINT PK_EMP_SECOND_ENO PRIMARY KEY,
...
VALUES (8000, -200); -- 에러 발생

-- 테이블 삭제
DROP TABLE EMP_SECOND;

-- 5) DEFAULT 제약 조건 :
-- 정의 : NULL 값이 INSERT 될 때 기본적으로 다른 값으로 생성하는 제약조건
CREATE TABLE EMP_SECOND(
    ENO NUMBER(4) CONSTRAINT PK_EMP_SECOND_ENO PRIMARY KEY,
...
-- 예제 2) 테이블 생성 후에 외래키(참조키:FK) 추가하기 : EMP_COPY
-- 사용법 : ALTER TABLE 테이블명
...
-- 참고사항 : 1) 실무에서 외래키(참조키 : FK) 관련 논란 존재
--            (1) DB에서 참조키를 만들자 파 :
--            (1-1) DB 성능이 저하됨
--            (1-2) DB 테이블에 테스트 데이터 만들 때 제약이 있음
--            (2) DB에서 참조키를 만들지말자 파 :
--            (2-1) 참조키 : JAVA에서 코딩으로 참조키 역할하게 만듦
