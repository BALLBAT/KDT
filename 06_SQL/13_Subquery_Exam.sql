...
-- 대상 : EMPLOYEE
-- 1-1) (사원번호가(ENO) 7788인 사원) : SCOTT, ANALYST
...
-- 대상 : EMPLOYEE
-- 2-1) 사원번호가 7499인 사원 : ALLEN, 1600
SELECT ENAME, JOB, SALARY FROM EMPLOYEE
WHERE ENO = '7499';

-- 2-2) 1600 보다 급여가 많은 사원의 이름, 담당업무 : 
SELECT ENAME, JOB FROM EMPLOYEE
WHERE SALARY > 1600;
...
-- 3-2) 800 급여를 받는 사원의 이름, 담당 업무, 급여를 표시하세요.
SELECT ENAME, JOB, SALARY FROM EMPLOYEE
WHERE SALARY = 800;
...
-- 4-1) 평균 급여가 가장 적은 업무? : CLERK, 1038
SELECT JOB, MIN(ROUND(AVG(SALARY))) FROM EMPLOYEE
GROUP BY JOB;

-- 4-2) 그 급여에 해당되는 직급, 평균급여를 출력 : 1038
...
-- 단, 서브쿼리를 이용하세요
-- 사원테이블 : EMPLOYEE
-- 7-1) 매니저가 있는 사원의 사원번호 :
-- 7369, 7499, 7521, 7566, 7654, 7698, 7782,
-- 7788, 7844, 7876, 7900, 7902, 7934
SELECT ENO FROM EMPLOYEE
...
--    오름차순으로 정렬하세요.
-- 사원테이블 : EMPLOYEE
-- 9-1) 평균 급여 찾기 : 2073.214285714285714285714285714285714286
SELECT AVG(SALARY) FROM EMPLOYEE;

-- 9-2) 평균 급여 보다 많은 급여를 받는 사원들
SELECT ENAME FROM EMPLOYEE
...
FROM EMPLOYEE   EM
    ,DEPARTMENT DE
WHERE EM.DNO = DE.DNO
AND EM.JOB = (SELECT JOB FROM EMPLOYEE
WHERE ENAME = 'ALLEN');
...
WHERE EM.DNO = DE.DNO
AND DE. DNO = 10
AND EM.JOB NOT IN (SELECT JOB FROM EMPLOYEE
WHERE DNO = 30);