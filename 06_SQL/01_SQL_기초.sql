...
--                  2) 명령어의 끝은 ;(세미콜론)
-- 3) 테이블 : 컴퓨터 폴더와 비슷. DB의 가장 작은 공간
--      예) 부서정보 테이블, 사원정보 테이블 등

-- 예제 1) 부서정보 테이블(DEPARTMENT)을 전체 조회 하세요
SELECT * FROM DEPARTMENT;

-- 4) 테이블 구조 : 세로 - 컬럼(정보의 각 소제목들이 표현)
--      예) Department 테이블 구조 : DNO(부서번호), DNAME(부서명), LOC(부서위치) => 컬럼
--                                 10          ACCOUNTING    NEW YORK    => 데이터(정보)

-- 연습 1) 사원정보 테이블(EMPLOYEE)을 전체 조회 하세요
-- 조회 사용법) SELECT * FROM 테이블명;
-- 조회 사용법) SELECT 컬럼명, 컬럼명2, ... FROM 테이블명;
-- * : 모든 컬럼
-- 실행 단축키 : 1) 1개 실행(ctrl + enter), 2) 전체 실행(F5)
SELECT * FROM EMPLOYEE;

-- 예제 2)사원 테이블(EMPLOYEE)의 컬럼 중에서 ENO, ENAME 컬럼만 조회하는 SQL 작성
-- 조회 사용법) SELECT 컬럼명, 컬럼명2, ... FROM 테이블명;
...
-- 월급 컬럼  : SALARY
-- 사원명 컬럼 : ENAME
-- 사원테이블에서 사원명과 1년 연봉을 화면에 출력해보세요, 산술연산자를 이용하면 됩니다.
SELECT ENAME, SALARY*12 FROM EMPLOYEE;

-- 예제 3) 사원테이블에서 COMMISSION(인센, 보너스)을 더한 연봉 구하기
-- 사원테이블 : EMPLOYEE
-- 월급 컬럼  : SALARY
-- 사원명 컬럼 : ENAME
-- 상여금 컬럼 : COMMISSION
-- 문제점 : DB에서 NULL은 아무것도 없는 값을 의미, 특징은 산술연산이 안됨 : NULL * 숫자 = NULL
-- 해결 : NULL -> 숫자로 바꾸는 함수 사용해서 개선 가능
-- NVL(컬럼, 0) => 컬럼의 NULL 값은 0으로 변경됨
-- 컬럼의 자료형 : 문자열, 숫자. 날짜 자료형 등
SELECT ENAME, SALARY*12+COMMISSION FROM EMPLOYEE;

-- 예제 3 개선 : COMMISSION 컬럼에 NULL겂이 존재함(산술연산 안됨)
SELECT ENAME, SALARY*12+NVL(COMMISSION, 0) FROM EMPLOYEE;

-- 예제 4) 위의 연봉 정보 컬럼에 별명 붙이기
...
-- 사용법 : SELECT DISTINCT 컬러명 FROM 테이블명;
