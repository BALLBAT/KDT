...
-- 목적 : 1) 보안을 유지하기 위해 사용
--           예) 실무 : 사원테이블 (급여:SALARY, 대외비)
--        2) SQL문을 미리 코딩 해놓고 재활용 : 코딩 생산성 향상
-- 예제 1) JOB 이 SALESMAN 인 사원테이블에서 
--       ENO(사원번호), ENAME(사원명), DNO(부서번호), JOB(직위) 컬럼만 화면에 출력하는 뷰생성
-- 사용법 : CREATE OR REPLACE 뷰이름(컬럼명, 컬럼명2, ...)
--          AS SELECT 원본컬럼명, 원본컬럼명2, ...
--          FROM 테이블명
--          WHERE 조건식;
-- 참고) 뷰이름(컬럼명, 컬럼명2, ...) => 뷰 이름 : 생략 가능(컬럼명)
...
-- *) (심화) 뷰에 INSERT 가능?
-- 예제 2) VW_EMP_JOB에 INSERT하기
-- 뷰 컬럼 : ENO, ENAME, DNO, JOB
