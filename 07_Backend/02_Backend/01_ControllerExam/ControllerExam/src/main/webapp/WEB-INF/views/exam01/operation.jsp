...
<body>
<%-- 복습 : container 클래스 : 가상의 페이지 중앙 정렬 --%>
<div class="container">
    <h2>산술연산자 / 테스팅</h2>
<%-- 부트스트랩 복습 : ms-3 클래스 : m(margin), s(start==left) : 왼쪽 마진 --%>
<%-- 부트스트랩 복습 : mt-3 클래스 : m(margin), t(top) : 위쪽 마진 --%>
    <div class="ms-3 mt-3">
        <p>문자열 : ${"문자열 출력됩니까?"}</p>
<%-- <p>문자열 더하기 : ${"문자열1" + "문자열2"}</p> --%>
        <p>${"문자열1"} ${"문자열2"}</p>
<%-- 문자열 "10" : 자동으로 숫자로 변환됨 --%>
        <p>덧셈("10") : ${strNum + 10}</p>
<%-- 사칙연산 테스트 : --%>
...
<%-- 비교연산(==같다, !=같지않다, >, <) 테스트 : --%>
        ...
        <p>크거나작음(10) : ${iNum <= 100}</p>

<%-- 3항연산자(조건문) : (조건식)? "실행1" : "실행2" --%>
        <p>조건(3항연산자) : ${(iNum==100)? "100" : "100이 아님"}</p>
